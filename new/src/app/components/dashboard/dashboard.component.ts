...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    ).subscribe((searchResult: SearchResult | undefined) => {
      if (searchResult) {
  ...
  goToPage(page: number) {
    this.goToPageSubscription = this.ws.searchWords(this.term, page).subscribe((searchResult: SearchResult | undefined) => {
      if (searchResult) {
        this.words = searchResult.results.data;
        this.doPagination(searchResult);
      }
